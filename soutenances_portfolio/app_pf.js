...
function ghFilePath() {
  return `soutenances_portfolio/donnees_pf.json`;
}
...
async function loadHorairesFromGitHub() {
  ...
  try {
    const url = `https://api.github.com/repos/${GH_OWNER}/${GH_REPO}/contents/soutenances_portfolio/horaires_pf.json?ref=${GH_BRANCH}`;
    const resp = await fetch(url, {
      headers: {
        Authorization: `token ${cfg.token}`,
        Accept: "application/vnd.github.v3+json",
...
async function saveCfgModal() {
  ...
  try {
    const ts = new Date().toLocaleString("fr-FR");
    await saveJsonToGitHub(
      "soutenances_portfolio/horaires_pf.json",
      JSON.stringify(merged, null, 2),
...
async function loadFromServer() {
  try {
    // Tente de récupérer donnees_pf.json s'il est accessible à la racine
    const resp = await fetch("donnees_pf.json", { cache: "no-store" });
    if (resp.ok) {
      const text = await resp.text();
      applyJSON(text);
      console.log(
        "Sélections chargées depuis donnees_pf.json (dossier courant)",
      );
    }
  } catch (e) {
...
async function loadHoraires() {
  try {
    const resp = await fetch("horaires_pf.json", { cache: "no-store" });
    if (resp.ok) {
  ...
  try {
    const rawUrl = `https://raw.githubusercontent.com/${GH_OWNER}/${GH_REPO}/${GH_BRANCH}/soutenances_portfolio/horaires_pf.json`;
    ...
    console.warn("Impossible de charger horaires_pf.json", e);
  }
}

async function loadEtudiants() {
  try {
    const resp = await fetch("etudiants_pf.json", { cache: "no-store" });
    if (resp.ok) {
      const data = await resp.json();
    ...
    console.warn("etudiants_pf.json non trouvé.", e);
  }
}
/* ── Toast ───────────────────────────────────────────────────────── */
...
function exportXLSX() {
  ...
  document.querySelectorAll(".jcard").forEach((card) => {
    ...
    const badge =
      card
        .querySelector(".stable tbody tr td:last-child span")
        ?.textContent.trim() || "—";
  ...
  XLSX.writeFile(
    wb,
    `soutenances_portfolio_${PAGE_ID}_${new Date().toISOString().slice(0, 10)}.xlsx`,
  );
